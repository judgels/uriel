...
public final class UrielControllerUtils extends AbstractJudgelsControllerUtils {
    ...
    public void appendSidebarLayout(LazyHtml content) {
        ...
        if (UrielUtils.isGuest()) {
            ...
            content.appendLayout(c -> isLoggedOutLayout.render(jophielClientAPI.getUserIsLoggedInAPIEndpoint(), org.iatoki.judgels.jophiel.controllers.routes.JophielClientController.logout(ControllerUtils.getCurrentUrl(Http.Context.current().request())).absoluteURL(Http.Context.current().request(), Http.Context.current().request().secure()), "lib/jophielcommons/javascripts/isLoggedOut.js", UrielUtils.getRealUserJid(), c));
        }
    }
