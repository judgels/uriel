...
import org.iatoki.judgels.uriel.controllers.forms.ContestClarificationCreateForm;
...
import org.iatoki.judgels.uriel.controllers.forms.ContestClarificationUpdateForm;
...
public class ContestClarificationController extends BaseController {
    ...
    public Result listScreenedClarifications(long contestId, long pageIndex, String orderBy, String orderDir, String filterString) throws ContestNotFoundException {
        ...
        if (ContestControllerUtils.getInstance().isAllowedToEnterContest(contest)) {
            ...
            contestService.readContestClarifications(IdentityUtils.getUserJid(), contestClarifications.getData().stream().filter(c -> c.isAnswered()).map(c -> c.getId()).collect(Collectors.toList()));
